...
def _heuristic_classify(query: str, prior_user_turns: list[str], carryover: dict | None) -> dict:
    ...
    # Predictive
    if "where will" in lower or "predict" in lower:
    ...
    # Financial planning
    if any(p in lower for p in ("save for", "retire", "fire plan", "on track", "child's college", "down payment")):
        agent = "financial_planning"
        if goal: entities["goal"] = goal
        if amount is not None and "earning" in lower: entities["amount"] = amount
...
def fake_llm():
    ...
    def _call(messages: list[dict[str, str]]) -> dict:
        ...
        return _heuristic_classify(current, prior, carryover)
    return _call
