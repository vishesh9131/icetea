...
def test_supervisor_resets_discussion_log_each_run():
    """Second collab call should not append onto the first run's discussion_log."""
    state = SessionState(session_id="s_re", turns=deque(maxlen=16))
    state.discussion_log.append({"agent": "stale", "round": 0, "utterance": "leftover"})
    classification = {"agent": "portfolio_health", "intent": "health", "entities": {}}
    hist1 = [{"role": "user", "content": "How is NVDA vs my portfolio?"}]
    _collect_supervisor(
        run_collaborative_supervisor(
            query="How is NVDA vs my portfolio?",
            session_id="s_re",
    ...
    n_after_first = len(state.discussion_log)
    assert n_after_first == 3
    hist2 = hist1 + [{"role": "assistant", "content": "ok"}, {"role": "user", "content": "Again — same ask?"}]
    _collect_supervisor(
        run_collaborative_supervisor(
            query="Again — same ask?",
            session_id="s_re",
            user_context={
                "user_id": "u1",
                "positions": [{"ticker": "NVDA", "quantity": 10, "avg_cost": 100, "currency": "USD"}],
    ...
    assert len(state.discussion_log) == n_after_first
