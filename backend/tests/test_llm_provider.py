...
from icetea.config import Settings, reset_settings_cache
from icetea.llm import LLMError, reset_llm_client


def test_settings_default_provider_is_openai(monkeypatch):
    # Pass a Settings instance directly so the .env file (which may set vllm
    # locally) doesnt poison the default-resolution check.
...
def test_openai_provider_without_key_raises_clearly():
    ...
    from icetea.llm import OpenAICompatClient
