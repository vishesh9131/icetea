...
def fake_llm():
    ...
    def _call(messages: list[dict[str, str]]) -> dict:
        ...
        if not isinstance(raw_td, dict):
            ...
            dt = raw_td.get("discussion_team")
            if isinstance(dt, list):
                merged["discussion_team"] = [str(x) for x in dt]
            merged["agent_team_discussion"] = bool(raw_td.get("agent_team_discussion"))
...
def _safe_env(monkeypatch):
    ...
    from icetea.config import reset_settings_cache
    from icetea.llm import reset_llm_client
...
def _isolated_session_store():
    ...
    from icetea import session as session_mod
...
def _block_real_market_data(monkeypatch):
    ...
    """
    from icetea import market_data
    ...
    from icetea.agents import portfolio_health as ph
