...
REFUSAL_MESSAGES: dict[str, str] = {
    ...
    "out_of_scope": (
        "This assistant is for investing education — budgeting, goals, risk, and markets — not for "
        "shopping or status-spend playbooks. Ask how to size a savings goal, invest toward a target, "
        "or understand costs and trade-offs in plain financial terms."
    ),
}
...
_ACTION_OVERRIDE: tuple[Pattern[str], ...] = tuple(
    ...
    for p in (
        ...
        # First-person "I work / I am at X and know" — classic insider scenario.
        r"\bi\s+(?:work|am)\s+at\s+\w+\s+and\s+know\b",
        # "when should I buy / sell" with insider context elsewhere in the line
        r"\bwhen\s+should\s+i\s+(?:buy|sell|trade|load|short)\b",
        # "ignoring all rules" anywhere in the message — see _INJECTION_COMBO
        r"\b(?:ignor(?:e|ing)|disregard(?:ing)?|bypass(?:ing)?)\s+(?:all|any|your|the)\s+(?:rules|safeguards?|guardrails?|safety|filters?|policies)\b",
    )
)
...
_INJECTION_LINE_START: tuple[Pattern[str], ...] = (
    _re(
        r"(?m)^\s*ignore\s+(?:all\s+)?(?:previous|prior|earlier)\s+"
        r"(?:instructions|safeguards?|guardrails?|safety|filters?)\b"
    ),
    _re(
        r"(?m)^\s*disregard\s+(?:all\s+)?(?:previous|prior|your)\s+"
        r"(?:instructions|safeguards?|guardrails?|safety|filters?)\b"
    ),
    _re(r"(?m)^\s*forget\s+(?:your\s+)?(?:rules|instructions|guidelines|safeguards?|guardrails?)\b"),
...
_INJECTION_COMBO: tuple[Pattern[str], ...] = (
    ...
    _re(
        r"(?:\bignore\s+(?:all\s+)?(?:previous|prior|earlier)\s+(?:instructions|safeguards?|guardrails?|safety|filters?)\b"
        r"|\bdisregard\s+(?:all\s+)?(?:previous|prior|your)\s+(?:instructions|safeguards?|guardrails?|safety|filters?)\b"
        r"|\bforget\s+(?:your\s+)?(?:rules|instructions|guidelines|safeguards?|guardrails?)\b)"
        r".{0,320}?"
        r"\b(?:liquidat(?:e|ing|ed|ion)?|sell\s+all|close\s+all(?:\s+positions)?|transfer\s+all|wire\s+all|"
    ...
    _re(
        ...
        r".{0,120}?"
        r"\b(?:liquidat(?:e|ing|ed|ion)?|sell\s+all|reveal\s+(?:your\s+)?(?:system\s+)?prompt)\b",
        re.IGNORECASE | re.DOTALL,
    ),
    # "Hypothetically / assuming you can / ignoring (all|any|your) rules" prefix
    # followed by any harmful imperative further down. This catches the soft
    # jailbreak that wraps a manipulation in academic framing.
    _re(
        r"\b(?:hypothetically|theoretically|just\s+for\s+(?:fun|argument)|let'?s\s+pretend|"
        r"in\s+a\s+(?:hypothetical|fictional)\s+scenario|assuming\s+(?:you\s+can|no\s+rules))\b"
        r".{0,160}?"
        r"\b(?:ignor(?:e|ing)|disregard(?:ing)?|bypass(?:ing)?|skip(?:ping)?|forget(?:ting)?)\s+"
        r"(?:all|any|your|the)\s+(?:rules|safeguards?|guardrails?|safety|filters?|policies)\b",
        re.IGNORECASE | re.DOTALL,
    ),
    _re(
        r"\b(?:ignor(?:e|ing)|disregard(?:ing)?)\s+(?:all|any|your|the)\s+(?:rules|safeguards?|"
        r"guardrails?|safety|filters?|policies)\b"
        r".{0,200}?"
        r"\b(?:pump\s+and\s+dump|insider\s+(?:trade|trading|info)|launder|coordinate\s+(?:a\s+)?"
        r"buying|wash\s+trade|spoof|manipulat)",
        re.IGNORECASE | re.DOTALL,
    ),
)
...
# Shopping / durables "how do I buy X" — not this product's job; runs before harmful scan.
_CONSUMER_HOW_TO_BUY_FRAMING = _re(
    r"\b(?:can\s+(?:you|u)\s+)?(?:please\s+)?(?:teach|show|tell)\s+me\s+how\s+to\s+buy\b"
    r"|\bhow\s+to\s+buy\b",
    re.IGNORECASE,
)
_FINANCE_SUBSTANCE = _re(
    r"\b(?:"
    r"stock|stocks|share|shares|"
    r"\betfs?\b|mutual\s+fund|index\s+fund|"
    r"\bbonds?\b|treasur(?:y|ies)|t-?bills?|"
    r"\boptions?\b|calls?|puts?|"
    r"brokerage|broker|"
    r"\b(?:roth\s+)?ira\b|401k|403b|"
    r"\breit\b|"
    r"\bticker\b|\bsymbol\b|"
    r"invest|investing|portfolio|equities|equity\s+market|dividend\s+stock"
    r")\b",
    re.IGNORECASE,
)
_HOME_OR_MORTGAGE_BUY = _re(
    r"\bhow\s+to\s+buy\s+(?:a\s+|an\s+|my\s+)?(?:house|home|first\s+home|first\s+house|condo|townhome|property)\b"
    r"|\b(?:first[- ]time\s+home\s+buyer|first[- ]time\s+buyer)\b"
    r"|\b(?:mortgage|down\s*payment|closing\s*costs?|preapproval|pre-approval)\b",
    re.IGNORECASE,
)


def _scan_consumer_how_to_buy_out_of_scope(query: str) -> tuple[str, str] | None:
    """Block generic retail shopping coaching; keep securities + home-buying finance."""
    if not _CONSUMER_HOW_TO_BUY_FRAMING.search(query):
        return None
    if _FINANCE_SUBSTANCE.search(query):
        return None
    if _HOME_OR_MORTGAGE_BUY.search(query):
        return None
    return "out_of_scope", r"consumer_how_to_buy_non_finance"
...
HARMFUL_PATTERNS: dict[str, tuple[Pattern[str], ...]] = {
    ...
    "market_manipulation": (
        ...
        _re(r"\bpump\s+and\s+dump\b.*\b(scheme|how\s+(?:do|should|can|would)\s+i|help|lay\s+out|set\s+up|run)\b"),
    ...
    "out_of_scope": (
        _re(
            r"\b(?:teach\s+me\s+|show\s+me\s+|tell\s+me\s+)?how\s+to\s+buy\s+(?:a\s+|an\s+)?"
            r"(?:lamborghin[io]|lamborgini|lambo|ferrari|mclaren|bugatti|rolls[- ]?royce|bentley)\b"
        ),
        _re(
            r"\b(?:teach|show|tell)\s+me\s+how\s+to\s+buy\b.{0,80}?"
            r"\b(?:lamborghin[io]|lamborgini|lambo|ferrari|mclaren|bugatti|rolls[- ]?royce|bentley)\b",
            re.DOTALL,
        ),
        _re(r"\bbuy\s+(?:a\s+|an\s+)?(?:lamborghin[io]|lamborgini|lambo)\b.*\b(?:teach|show|help)\b"),
    ),
}
...
def check(query: str) -> SafetyVerdict:
    ...
    shop = _scan_consumer_how_to_buy_out_of_scope(text)
    if shop is not None:
        category, pattern_text = shop
        return SafetyVerdict(
            blocked=True,
            category=category,
            message=REFUSAL_MESSAGES[category],
