...
from . import team_planner
from . import team_registry_bridge as team_reg
from .team_planner import TeamPlan
...
from .toolkits import planning_math as plan_math
...
# Each /v1/chat collab run should deliberate from a clean audit trail; old cycles
# stacked here used to leak into prompts and structured payloads (state drift).
_COLLAB_HISTORY_MAX_MESSAGES = 12
...
def _collab_conversation_tail(hist: list[dict[str, str]]) -> list[dict[str, str]]:
    """Last N chat lines for one collab episode — keeps repeat-query runs from snowballing."""
    cap = _COLLAB_HISTORY_MAX_MESSAGES
    if not hist or len(hist) <= cap:
        return hist
    return hist[-cap:]
...
def _portfolio_round(user_context: dict[str, Any]) -> dict[str, Any]:
    ...
    def _mcp(server_id: str, endpoint_id: str, **kwargs: Any) -> Any:
        res = invoke_mcp(server_id, endpoint_id, user_context=user_context, **kwargs)
        mcp_calls.append({"server_id": server_id, "endpoint_id": endpoint_id, "result": res})
        if isinstance(res, dict) and res.get("ok") and "data" in res:
            return res["data"]
        return res

    tools_out = {
        "list_positions": _mcp("mcp_portfolio_core", "positions", query="", tickers=[]),
        "summarize_allocation": _mcp("mcp_portfolio_core", "allocation", query="", tickers=[]),
        "flag_concentration": _mcp("mcp_portfolio_core", "concentration", query="", tickers=[]),
        "match_tickers_to_positions": _mcp(
            "mcp_portfolio_core",
            "match_tickers",
            query="",
            tickers=_extract_tickers("", positions),
        ),
        "snapshot_risk_profile": _mcp("mcp_portfolio_core", "risk_profile", query="", tickers=[]),
        "factor_overlap_stub": _mcp("mcp_risk_tax", "factor_overlap", query="", ticker=""),
        "_mcp_calls": mcp_calls,
    }
...
def _market_round(
    ...
    *,
    ...
    qinfo = _mcp("mcp_market_tape", "quote", query=query, ticker=primary)
    tools_out = {
        "get_quote": qinfo,
        "get_recent_returns": _mcp("mcp_market_tape", "returns_window", query=query, ticker=primary),
        "sector_benchmark_stub": _mcp("mcp_market_tape", "sector_benchmark", query=query, ticker=primary),
        "symbol_resolve": _mcp("mcp_market_tape", "resolve_symbol", query=query, ticker=primary),
        "market_hours_stub": _mcp("mcp_market_tape", "market_hours", query=query, ticker=primary),
        "book_risk_flags": _mcp("mcp_risk_tax", "book_risk_flags", query=query, ticker=primary),
        "_mcp_calls": mcp_calls,
    }
...
def _tax_math_round(
    ...
    *,
    ...
    mcp_calls: list[dict[str, Any]] = []
    res = invoke_mcp("mcp_risk_tax", "tax_gain_bundle", user_context=user_context, query=query, ticker=str(ticker))
    mcp_calls.append({"server_id": "mcp_risk_tax", "endpoint_id": "tax_gain_bundle", "result": res})
    bundle = (res.get("data") if isinstance(res, dict) and res.get("ok") else res) or {}
    utterance = bundle.get("summary_public") or ""
    internal = bundle.get("summary_internal") or ""
    return {
        "agent": "tax_math",
        "round": round_no,
        "tool_results": {"tax_gain_bundle": bundle, "_mcp_calls": mcp_calls},
        "utterance": utterance,
...
# ---------- retirement / multi-step planning panel ----------


def _planner_round(query: str, user_context: dict[str, Any], *, round_no: int = 1) -> dict[str, Any]:
    """Planner agent: anchor goal, list known vs assumed inputs. Pure deterministic."""
    bundle = plan_math.build_retirement_bundle(query=query, user_context=user_context)
    sig = bundle["inputs_detected"]
    assumptions = bundle["assumptions"]
    known_bits: list[str] = []
    if sig.get("age") is not None:
        known_bits.append(f"age {sig['age']}")
    if sig.get("horizon_years"):
        known_bits.append(f"horizon ~{sig['horizon_years']}y")
    if sig.get("annual_income") is not None:
        known_bits.append(f"annual income {sig['annual_income']:,.0f}")
    if sig.get("risk_profile"):
        known_bits.append(f"risk profile {sig['risk_profile']}")
    if sig.get("current_savings") is not None:
        known_bits.append(f"current savings {sig['current_savings']:,.0f}")
    if sig.get("monthly_income_target_parsed") is not None:
        known_bits.append(f"stated monthly target {sig['monthly_income_target_parsed']:,.0f}")
    known_line = ("Inputs available: " + ", ".join(known_bits)) if known_bits else "No explicit numbers given."
    assumed_line = (
        f"Assumptions used (labelled, not personal advice): replacement ratio "
        f"{int(assumptions['replacement_ratio_range'][0]*100)}–{int(assumptions['replacement_ratio_range'][1]*100)}%, "
        f"withdrawal rates {', '.join(f'{int(r*1000)/10}%' for r in assumptions['withdrawal_rates'])}, "
        f"long-term inflation {assumptions['inflation_long_term']*100:.1f}%, "
        f"risk profile for return path: {assumptions['risk_profile_used_for_returns']}."
    )
    utterance = (
        "Planner: locking down the question so the rest of the panel can reason with numbers. "
        + known_line
        + " "
        + assumed_line
        + " Missing inputs we will bracket instead of asking for: "
        + (", ".join(bundle.get("missing_inputs_for_higher_fidelity") or []) or "none")
        + "."
    )
    return {
        "agent": "planner",
        "round": round_no,
        "tool_results": {"planning_bundle": bundle},
        "utterance": utterance,
        "internal": {"deliberation": "[planner/anchor]", "facts": sig},
    }


def _projection_round(planner_peer: dict[str, Any], *, round_no: int = 2) -> dict[str, Any]:
    """Projection agent: corpus need + back-solved contributions across scenarios."""
    bundle = (planner_peer.get("tool_results") or {}).get("planning_bundle") or {}
    ib = bundle.get("income_target_band") or {}
    cb = bundle.get("required_corpus") or {}
    cs = bundle.get("contribution_solver") or {}
    currency = ((bundle.get("inputs_detected") or {}).get("base_currency") or "USD")

    bits: list[str] = []
    if ib.get("reference_pre_retirement_income_band"):
        ref = ib["reference_pre_retirement_income_band"]
        bits.append(
            "No salary on file — bracketing from labelled reference gross-income band "
            f"({plan_math._fmt_money(ref[0], currency)}–{plan_math._fmt_money(ref[1], currency)}/yr) "
            "× replacement ratios, same as a sketch-plan human would do."
        )
    bits.append(
        f"Income target band (today's {currency}): "
        f"{plan_math._fmt_money(ib.get('annual_low'), currency)}–{plan_math._fmt_money(ib.get('annual_high'), currency)}/yr "
        f"({plan_math._fmt_money(ib.get('monthly_low'), currency)}–{plan_math._fmt_money(ib.get('monthly_high'), currency)}/mo). "
        f"Source: {ib.get('source')}."
    )
    if cb.get("corpus_band_low") is not None:
        bits.append(
            f"Required corpus across {', '.join(f'{int(r*1000)/10}%' for r in cb.get('withdrawal_rates_used') or [])} "
            f"withdrawal rates: {plan_math._fmt_money(cb['corpus_band_low'], currency)}"
            f"–{plan_math._fmt_money(cb['corpus_band_high'], currency)}."
        )
    rows = cs.get("scenarios") or []
    if rows:
        chunks = []
        for r in rows:
            chunks.append(
                f"{r['scenario']} ({r['real_annual_return']:.1%} real): "
                f"{plan_math._fmt_money(r['monthly_contribution_low'], currency)}"
                f"–{plan_math._fmt_money(r['monthly_contribution_high'], currency)}/mo"
            )
        bits.append("Back-solved monthly contribution to hit the band — " + "; ".join(chunks) + ".")
    elif (bundle.get("inputs_detected") or {}).get("horizon_years") is None:
        bits.append("Contribution solve skipped until horizon years are known (parse query or set horizon_years).")

    utterance = "Projection: " + " ".join(bits)
    return {
        "agent": "projection",
        "round": round_no,
        "tool_results": {"projection_bundle": {"income_band": ib, "corpus": cb, "contributions": cs}},
        "utterance": utterance,
        "internal": {"deliberation": "[projection/math]", "facts": cs},
    }


def _scenario_round(planner_peer: dict[str, Any], *, round_no: int = 3) -> dict[str, Any]:
    """Scenario / risk agent: deterministic stress against the projection."""
    bundle = (planner_peer.get("tool_results") or {}).get("planning_bundle") or {}
    ...
    if not paths:
        utterance = (
            "Scenario: stress sleeves need both corpus band and horizon — "
            "add years-to-goal in query or user_context.horizon_years."
        )
        return {
            "agent": "scenario",
            "round": round_no,
            "tool_results": {"scenario_bundle": stress},
            "utterance": utterance,
            "internal": {"deliberation": "[scenario/insufficient]", "facts": {}},
        }
    bits: list[str] = []
    stressed_low = cb.get("corpus_band_low_sequence_stressed")
    stressed_high = cb.get("corpus_band_high_sequence_stressed")
    if stressed_low is not None:
        bits.append(
            f"Sequence-of-returns stress (corpus need expands by "
            f"{int((cb.get('sequence_stress_haircut') or 0)*100)}%): "
            f"{plan_math._fmt_money(stressed_low, currency)}–{plan_math._fmt_money(stressed_high, currency)}."
        )
    for p in paths:
        bits.append(
            f"{p['name'].replace('_', ' ')} (real {p['real_annual_return_used']:.1%}): "
            f"{plan_math._fmt_money(p['monthly_contribution_low'], currency)}"
            f"–{plan_math._fmt_money(p['monthly_contribution_high'], currency)}/mo to stay on track."
        )
    utterance = "Scenario: " + " ".join(bits)
    return {
        "agent": "scenario",
        "round": round_no,
        "tool_results": {"scenario_bundle": stress},
        "utterance": utterance,
        "internal": {"deliberation": "[scenario/stress]", "facts": stress},
    }


def _tax_account_round(query: str, user_context: dict[str, Any], *, round_no: int = 4) -> dict[str, Any]:
    """Qualitative tax / account-type agent — generic frame, not jurisdiction-specific advice."""
    risk = str(user_context.get("risk_profile") or "").lower() or "unspecified"
    has_book = bool(port_tools.list_positions(user_context))
    bits = [
        "Tax / account framing (generic, jurisdiction-agnostic):",
        "Sequence and account location matter — withdrawals from tax-deferred vs taxable vs Roth-style "
        "accounts can change after-tax cash flow even at the same gross corpus.",
        "Contribution side: any employer-match or tax-advantaged wrapper you have access to is usually the "
        "first dollar to deploy before taxable brokerage.",
        f"Risk profile on file: {risk}. Sequence risk near retirement is what argues for a glidepath, "
        "not a static allocation.",
    ]
    if has_book:
        bits.append(
            "Existing taxable positions feed into harvest/realisation timing; that is a follow-up tax-collab "
            "question, not part of this planner's corpus math."
        )
    utterance = " ".join(bits)
    return {
        "agent": "tax_account",
        ...
        "tool_results": {},
        "utterance": utterance,
        "internal": {"deliberation": "[tax/account/generic]", "facts": {"risk_profile": risk}},
    }


def _retirement_synthesis_round(
    query: str,
    planner_peer: dict[str, Any],
    projection_peer: dict[str, Any],
    scenario_peer: dict[str, Any],
    tax_peer: dict[str, Any],
    *,
    round_no: int = 5,
) -> dict[str, Any]:
    bundle = (planner_peer.get("tool_results") or {}).get("planning_bundle") or {}
    summary = plan_math.render_planner_summary(
        bundle,
        currency=((bundle.get("inputs_detected") or {}).get("base_currency") or "USD"),
    )
    closer = (
        "\n\nNext step that actually moves the plan forward: pick a monthly contribution from the bracket above "
        "you can sustain today, automate it, and revisit when one of these inputs changes — current savings, "
        "income, or your monthly retirement spend target."
    )
    utterance = summary + closer + "\n\n" + conv_tools.toolkit_disclaimer_block()
    return {
        "agent": "synthesis",
        "round": round_no,
        "tool_results": {},
        "utterance": utterance,
        "internal": {"deliberation": "[synthesis/retirement]", "facts": bundle.get("assumptions") or {}},
        "collaboration_meta": {"mode": "retirement_planning"},
    }


def _compose_retirement_user_message(entries: list[dict[str, Any]], query: str) -> str:
    """Final user-facing message — synthesis last, panel context above."""
    chunks: list[str] = [f"You asked: {query.strip()}", ""]
    for e in entries:
        u = (e.get("utterance") or "").strip()
        if u:
            chunks.append(u)
...
def _discussion_for_response(entries: list[dict[str, Any]], *, utterance_max_chars: int = 4500) -> list[dict[str, Any]]:
    """Client-safe rows: captions only — tool names and raw traces stay on disk via SessionState."""
    rows: list[dict[str, Any]] = []
    for e in entries:
        raw_u = (e.get("utterance") or "").strip()
        if len(raw_u) > utterance_max_chars:
            raw_u = raw_u[: max(0, utterance_max_chars - 1)].rsplit(" ", 1)[0] + "…"
        row: dict[str, Any] = {
            "round": e.get("round"),
            "agent": e.get("agent"),
            "utterance": raw_u,
...
def _shared_facts_from_team_entries(entries: list[dict[str, Any]]) -> dict[str, Any]:
    pf: dict[str, Any] = {}
    mf: dict[str, Any] = {}
    for e in entries:
        if e.get("agent") == "portfolio":
            pf = _portfolio_facts(e.get("tool_results") or {})
        elif e.get("agent") == "market":
            mf = _market_facts(e.get("tool_results") or {})
    return {"portfolio_facts": pf, "market_facts": mf}


def _extra_context_for_discussion(entries: list[dict[str, Any]]) -> dict[str, Any]:
    for e in entries:
        if e.get("agent") == "tax_math":
            return {"tax_gain_bundle": (e.get("tool_results") or {}).get("tax_gain_bundle") or {}}
    return {"tax_gain_bundle": {}}


def _deterministic_discussion_turn(speaker: str, cycle: int, prior_ct: int) -> dict[str, Any]:
    base = f"[discussion cycle {cycle}] Thread depth {prior_ct} — "
    tails = {
        "portfolio": base + "Book structure still drives outcomes; pushing back if tape-only replies ignore weight.",
        "market": base + "Levels matter but mean-revert; debating Risk if they ignore horizon.",
        "tax_math": base + "Stub math only — not filing advice; disagree if someone over-reads notional.",
        "risk": base + "Concentration + shock is the tandem; willing to argue Momentum on half-life.",
        "momentum": base + "Trend can persist; still acknowledging single-name dominance from Portfolio.",
    }
    msg = tails.get(speaker, base + f"{speaker} adds a consistency pass across prior thread lines.")
    return {
        "reasoning_trace": [f"stub_{speaker}_c{cycle}"],
        "confidence_0_100": 55,
        "stance_headline": f"{speaker} replies in thread",
        "answer": msg,
        "abstain": False,
        "abstain_reason": "",
    }
...
def _compose_thread_synthesis_offline(transcript: list[dict[str, Any]], query: str) -> str:
    lines = [
        f"(c{t.get('cycle')}) {t.get('registry_agent') or t.get('agent')}: {(t.get('text') or '')[:260]}"
        for t in transcript
    ]
    body = (
        "Synthesis (offline stub): specialists debated in-thread; this is not a live committee record.\n"
        f"User ask: {query.strip()}\n\n"
        + "\n".join(lines)
    )
    return body + "\n\n" + conv_tools.toolkit_disclaimer_block()


async def _iter_agent_team_discussion(
    *,
    query: str,
    user_context: dict[str, Any],
    ...
    """Tool bootstrap, then multi-cycle thread over orchestrator-planned registry slots."""
    slots = team_plan.slots
    ...
    portfolio_peer = _portfolio_round(user_context)
    pf_emitted = mk_emitted = tax_emitted = risk_stub_emitted = False
    strat_seen = 0

    for slot in slots:
        reg = slot.registry_agent
        if reg == "portfolio_health" and not pf_emitted:
            pf_emitted = True
            rnd += 1
            portfolio_peer["round"] = rnd
            entries.append(portfolio_peer)
            state.discussion_log.append(portfolio_peer)
            yield {
                "type": "meta",
                "stage": "agent_discussion",
                "round": portfolio_peer["round"],
                "agent": portfolio_peer["agent"],
                "utterance_preview": _preview(portfolio_peer),
                "reasoning_agent": "portfolio",
            }
        elif reg == "market_research" and not mk_emitted:
            mk_emitted = True
            rnd += 1
            row = _market_round(user_context, query, portfolio_peer, round_no=rnd)
            entries.append(row)
            state.discussion_log.append(row)
            yield {
                "type": "meta",
                "stage": "agent_discussion",
                "round": row["round"],
                "agent": row["agent"],
                "utterance_preview": _preview(row),
                "reasoning_agent": str(row.get("agent")),
            }
        elif reg == "investment_strategy":
            strat_seen += 1
            want_tax = not tax_emitted and bool(port_tools.list_positions(user_context))
            if want_tax and (
                slot.voice == "tax_math"
                or (strat_seen == 1 and team_planner.query_sounds_taxish(query))
            ):
                tax_emitted = True
                rnd += 1
                row = _tax_math_round(user_context, query, classification, portfolio_peer, round_no=rnd)
                entries.append(row)
                state.discussion_log.append(row)
                yield {
                    "type": "meta",
        ...
        elif reg == "risk_assessment" and not risk_stub_emitted:
            risk_stub_emitted = True
            rnd += 1
            row = {
                "agent": "risk",
                "round": rnd,
                "tool_results": {},
                "utterance": "Risk lens queued — joining the thread next.",
                "internal": {"deliberation": "[risk/bootstrap]", "facts": {}},
            }
            entries.append(row)
            state.discussion_log.append(row)
            yield {
    ...
    shared = _shared_facts_from_team_entries(entries)
    extra = _extra_context_for_discussion(entries)

    team_labels = [{"registry_agent": s.registry_agent, "voice": s.voice} for s in slots]

    for cycle in range(1, DISCUSSION_CYCLES + 1):
        for slot in slots:
            rnd += 1
            lm: dict[str, Any] = {}
            text = ""
            reg_out = None
            collab_key = team_planner.collab_speaker_key(slot, query)
            try:
                reg_out = await team_reg.collect_registry_panel_turn(
                    registry_agent=slot.registry_agent,
                    voice=slot.voice,
                    query=query,
                ...
                logger.warning("Registry panel turn skipped (%s): %s", slot.registry_agent, exc)

            if reg_out is not None:
                text = reg_out.text.strip()
                lm = {
                    "reasoning_trace": [f"registry:{reg_out.registry_agent}"],
                    "confidence_0_100": 72,
                    "stance_headline": f"{slot.voice} ({reg_out.registry_agent})",
                    "answer": text,
                    "abstain": False,
                    "abstain_reason": "",
                }
            elif _use_llm_collab(llm):
                assert isinstance(llm, LLMClient)
                try:
                    lm = await collab_llm.run_discussion_turn(
                        llm,
                        speaker_role=collab_key,
                        query=query,
                        transcript=list(transcript),
                        shared_facts=shared,
                        extra_context=extra,
                    ...
                    logger.warning("Discussion turn LLM degraded (%s): %s", slot.registry_agent, exc)
                    lm = _deterministic_discussion_turn(collab_key, cycle, len(transcript))
                text = str(lm.get("answer") or lm.get("stance_headline") or "").strip()
            else:
                lm = _deterministic_discussion_turn(collab_key, cycle, len(transcript))
                text = str(lm.get("answer") or lm.get("stance_headline") or "").strip()
            ...
            row = {
                ...
                "collaboration_meta": {
                    ...
                    "speaker": slot.voice,
                    "registry_agent": slot.registry_agent,
                    "turn_source": "registry_agent" if reg_out is not None else "collab_or_stub",
                },
            }
            entries.append(row)
            state.discussion_log.append(row)
            yield {
                ...
                "agent": slot.registry_agent,
                "utterance_preview": _preview(row),
                "reasoning_agent": slot.registry_agent,
            }

    rnd += 1
    syn_entry: dict[str, Any] = {
        "agent": "synthesis",
        "round": rnd,
        "tool_results": {},
        "utterance": "",
    ...
    yield {
        ...
        "agent": "synthesis",
        "utterance_preview": "Chair synthesizing multi-agent thread…",
    ...
    if _use_llm_collab(llm):
        ...
        try:
            ...
            logger.warning("Thread chair stream degraded: %s", exc)
            buf = []

    disc = conv_tools.toolkit_disclaimer_block()
    if buf:
        body = "".join(buf).strip()
        final_text = body if disc.lower() in body.lower() else body + "\n\n" + disc
    else:
        final_text = _compose_thread_synthesis_offline(transcript, query)
        if not streamed:
            yield {"type": "data", "delta": final_text}

    syn_entry["utterance"] = final_text
    syn_entry["collaboration_meta"] = {
        "mode": "agent_team_synthesis",
        "selected_team": team_labels,
        "team_plan_source": team_plan.source,
        "discussion_cycles": DISCUSSION_CYCLES,
    }
    if team_plan.rationale:
        syn_entry["collaboration_meta"]["team_plan_rationale"] = team_plan.rationale
    ...
    structured: dict[str, Any] = {
        ...
        "collaboration_meta": syn_entry["collaboration_meta"],
        "collaboration_signals": _collaboration_signals(entries),
        "multiagent_llm": bool(_use_llm_collab(llm)),
        "agent_thread_transcript": transcript,
        "selected_team": team_labels,
    }
...
async def run_collaborative_supervisor(
    *,
    ...
    Each call clears ``discussion_log`` first so a new user turn does not stack
    onto prior panel cycles (same session_id, repeated asks).
    """
    state.discussion_log.clear()
    collab_history = _collab_conversation_tail(conversation_history)

    plan_team: TeamPlan | None = None
    ...
    if orch_prof == "tax_market_synthesis":
    ...
    elif orch_prof == "retirement_planning":
        meta_hint["orchestration_plan"] = [
            "planner",
            "projection",
            "scenario",
            "tax_account",
        ...
        meta_hint["orchestration_profile"] = orch_prof
    elif orch_prof == "agent_team_discussion":
        plan_team = await team_planner.plan_discussion_team(
            query=query,
            classification=classification,
            user_context=user_context,
            llm=llm,
        )
        meta_hint["orchestration_plan"] = [s.registry_agent for s in plan_team.slots] + [
            f"discussion_x{DISCUSSION_CYCLES}",
            "synthesis_chair",
        ]
        meta_hint["selected_team"] = [
            {"registry_agent": s.registry_agent, "voice": s.voice} for s in plan_team.slots
        ]
        meta_hint["team_plan_source"] = plan_team.source
        if plan_team.rationale:
            meta_hint["team_plan_rationale"] = plan_team.rationale
        meta_hint["orchestration_profile"] = orch_prof
    if _use_llm_collab(llm):
    ...
    if classification.get("orchestration_profile") == "tax_market_synthesis":
        ...
        structured = {
            ...
            "discussion_rounds": len(entries),
            "discussion_log": light,
            "prior_user_queries": conv_tools.toolkit_prior_user_turns(collab_history)[:-1],
            "collaboration_meta": {"mode": "tax_market_synthesis"},
            "collaboration_signals": _collaboration_signals(entries),
    ...
    if classification.get("orchestration_profile") == "retirement_planning":
        p_planner = _planner_round(query, user_context, round_no=1)
        entries.append(p_planner)
        state.discussion_log.append(p_planner)
        yield {
            "type": "meta",
            "stage": "agent_discussion",
            "round": p_planner["round"],
            "agent": p_planner["agent"],
            "utterance_preview": _preview(p_planner),
        }

        p_proj = _projection_round(p_planner, round_no=2)
        entries.append(p_proj)
        state.discussion_log.append(p_proj)
        yield {
            "type": "meta",
            "stage": "agent_discussion",
            "round": p_proj["round"],
            "agent": p_proj["agent"],
            "utterance_preview": _preview(p_proj),
        }

        p_scn = _scenario_round(p_planner, round_no=3)
        entries.append(p_scn)
        state.discussion_log.append(p_scn)
        yield {
            "type": "meta",
            "stage": "agent_discussion",
            "round": p_scn["round"],
            "agent": p_scn["agent"],
            "utterance_preview": _preview(p_scn),
        }

        p_tax = _tax_account_round(query, user_context, round_no=4)
        entries.append(p_tax)
        state.discussion_log.append(p_tax)
        yield {
            "type": "meta",
        ...
        p_syn = _retirement_synthesis_round(query, p_planner, p_proj, p_scn, p_tax, round_no=5)
        entries.append(p_syn)
        state.discussion_log.append(p_syn)
        yield {
            "type": "meta",
        ...
        final_text = _compose_retirement_user_message(entries, query)
        yield {"type": "data", "delta": final_text}

        light = _discussion_for_response(entries)
        planning_bundle = (p_planner.get("tool_results") or {}).get("planning_bundle") or {}
        structured = {
            "agent": "multiagent_orchestrator",
            "implemented": True,
            "intent": classification.get("intent"),
            ...
            "collaboration_signals": _collaboration_signals(entries),
            "multiagent_llm": False,
            "retirement_plan_bundle": planning_bundle,
        }
        if len(state.discussion_log) > 32:
            state.discussion_log[:] = state.discussion_log[-32:]
        yield {"type": "structured", "payload": structured}
    ...
    if classification.get("orchestration_profile") == "agent_team_discussion":
        assert plan_team is not None
        async for ev in _iter_agent_team_discussion(
            query=query,
            user_context=user_context,
            conversation_history=collab_history,
            state=state,
            classification=classification,
            llm=llm,
            team_plan=plan_team,
    ...
    if _use_llm_collab(llm) and rounds >= 3:
        ...
        collab_meta = {
            ...
            **tension,
            "confidence_fields_are_single_pass_self_ratings": True,
        }
    ...
    if rounds >= 3:
        ...
        p3 = _conversation_round(collab_history, query, last_intent, p1, market_peer)
        entries.append(p3)
