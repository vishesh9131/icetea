...
class SessionState:
    ...
    # Small persisted cache of user context so follow-ups don't lose the book
    # when the client doesn't resend positions every turn.
...
class SessionStore(Protocol):
    ...
    def update_context_cache(self, session_id: str, *, user_context: dict[str, Any]) -> None: ...
    def append_turn(self, session_id: str, role: str, content: str) -> None: ...
    def flush_session(self, session_id: str) -> None: ...
    def reset(self, session_id: str) -> None: ...


def _slim_user_context_for_cache(user_context: dict[str, Any]) -> dict[str, Any]:
    """Persist only stable, small fields (never MCP catalog)."""
    uc = user_context or {}
    slim: dict[str, Any] = {}
    for k in ("user_id", "risk_profile", "base_currency", "age", "annual_income", "current_savings"):
        if uc.get(k) is not None:
            slim[k] = uc.get(k)
    # positions can be large; cap and store only common keys
    pos = uc.get("positions")
...
class InMemorySessionStore:
    ...
    def update_context_cache(self, session_id: str, *, user_context: dict[str, Any]) -> None:
        ...
        with self._lock:
            slim = _slim_user_context_for_cache(user_context)
            if not slim:
                return
            # merge; positions overwrite if provided
            state.context_cache.update({k: v for k, v in slim.items() if k != "positions"})
            if slim.get("positions"):
                state.context_cache["positions"] = slim["positions"]
...
class FileBackedSessionStore:
    ...
    def _to_json(self, state: SessionState) -> dict:
        return {
            ...
            "context_cache": dict(state.context_cache),
        }
