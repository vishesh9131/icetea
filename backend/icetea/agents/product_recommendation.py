...
Uses the shared web toolkit for search/fetch. ``user_context`` may also carry
the MCP catalog (4 shim servers × 5 endpoints) for model awareness.
"""
...
class ProductRecommendationAgent:
    ...
    async def run(
        ...
        *,
        ...
        mcp = user_context.get("mcp_servers")
        if mcp:
            user_blob += "\nMCP_SERVERS (4×5 endpoints, in-process shim):\n" + json.dumps(mcp, indent=2, default=str)[:8000]
