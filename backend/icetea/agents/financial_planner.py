...
Inputs:
- You receive PLAN_FACTS_JSON: the user's words plus light profile hints (risk label, currency, holdings list).
- The server ALREADY ran deterministic retirement math when enough inputs were present — that is in ``retirement_bundle``
  (income target band, corpus need across withdrawal rates, monthly contribution scenarios, stress paths, and the labelled assumptions).
- The server also emitted ``plan_scaffold``: ordered steps with titles — expand those into plain language, dont replace them with unrelated themes.

Rules:
- USE the numbers in ``retirement_bundle`` when present — do not say "we can't calculate" if the bundle has a corpus band.
  Anchor on income_target_band, corpus, and the monthly contribution scenarios. ALWAYS state the assumptions you are leaning on
  (replacement ratio, withdrawal rate, real return, horizon) so it is transparent.
- If a number IS missing, name the SINGLE highest-value missing input (from ``missing_inputs_for_higher_fidelity``) and STILL
  give a bracketed answer based on the other inputs. Do not ask for a long list of fields before reasoning.
- Do NOT invent inputs the bundle did not produce — never fabricate current savings, contribution rate, or specific tax bracket numbers.
- Do NOT claim Monte Carlo or regulated suitability ran. The math is deterministic FV-annuity + withdrawal-rate corpus only.
- Monthly passive income targets imply defining spend in today's money vs inflation — say that plainly.
- Warm tone, five or six short paragraphs max.
...
def _build_plan_facts(
    *,
    ...
    retirement_bundle = plan_math.build_retirement_bundle(query=query, user_context=user_context)
    ...
    return {
        ...
        "retirement_bundle": retirement_bundle,
        "deterministic_summary": plan_math.render_planner_summary(
            retirement_bundle,
            currency=str(user_context.get("base_currency") or "USD"),
        ),
        "engines_present": {
            "monte_carlo": False,
...
def _offline_narrative(facts: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        "### Planner outline (deterministic scaffold)",
        "",
        f"{hz_line}; theme: **{facts.get('goal_theme')}**.",
        f"Positions on file: **{facts.get('position_count_on_file')}** lines.",
        "",
    ]

    summary = (facts.get("deterministic_summary") or "").strip()
    if summary:
        lines.append("**Numeric projection (textbook formulas, labelled assumptions):**")
        lines.append("")
        lines.append(summary)
        lines.append("")

    lines.append("**Planning checkpoints:**")
    for block in facts.get("plan_scaffold") or []:
        title = block.get("title") or block.get("id")
        lines.append(f"- {title}")
    lines.append("")

    missing = facts.get("missing_inputs_for_quant_plan") or []
    if missing:
        lines.append("**Highest-value input to tighten next:**")
